...
The output is valid GFM.  Bold, italic, code, links, images, and
strikethrough markers are duplicated on each continuation line.
When wrapping produces multi-line rows, empty rows are inserted
between data rows for readability.  In buffers where
...
- ~(markdown-table-wrap TEXT WIDTH &optional MAX-CELL-HEIGHT STRIP-MARKUP COMPACT)~
  ...
  Rewrite a pipe table to fit WIDTH.  Returns the wrapped string,
  or TEXT unchanged when it already fits.  STRIP-MARKUP measures
...
- ~(markdown-table-wrap-unwrap TEXT)~
  ...
  Merge continuation rows back into logical rows.  Idempotent.
